...
from preprocess.Dataset_orignial import get_dataloader
...
def prepare_dataloader(config):
    ...
    if config.data != 'PM2.5-multiclass':
        tr_statistics = []
    else:
        with open(config.data + 'tr_statistics.pkl', 'rb') as f:
            tr_statistics = pickle.load(f, encoding='latin-1')
    trainloader = get_dataloader(train_data, config, shuffle=True)
    testloader = get_dataloader(test_data, config, shuffle=False)

    return trainloader, testloader, num_types, d_covar, tr_statistics


def visual(fig_name_prefix, pred_time, gt_time, tr_statistics, err_dict, feature_importance_inner, feature_importance_outer):
    ...
    ax2.barh(feature_importance_inner['feature_name'], feature_importance_inner['global_attention'], label='global-importance')
    ax2.set_title('Global-importance')
    ...
    for i, v in enumerate(feature_importance_inner['global_attention'].to_numpy()):
        ax2.text(v + 0.01, i, str(format(v, '.4f')), color='green', fontweight='bold')
    plt.savefig(fig_name_prefix + ' Feature Importance inner.png', dpi=300)
    plt.close()
    ...
    feature_importance_inner.to_csv(fig_name_prefix + ' Feature Importance inner-instance-level.csv')
...
def train_epoch(model, training_data, optimizer, pred_loss_func, epoch, tr_statistics, config):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        loss = (-1) * log_prob.sum() * (10**(-mode_1)) + pred_loss * (10**(-mode_2+1)) + se * (10**(-mode_3)) # balance the loss
    ...
    if config.data == 'data/synthetic_/':
        feature_order = ['feature_{}'.format(i) for i in range(model.d_covar)]

    Fi_inner = pd.DataFrame(columns=['feature_name', 'instance_level', 'global_attention'])
    
    Fi_inner['feature_name'] = feature_order
    Fi_inner['instance_level'] = instance_level
    Fi_inner['global_attention'] = global_attention
    ...
    c_m = -42  # confusion_matrix(Pred_type_total, Next_type_total)
    ...
    if epoch % 2 == 0 or epoch == 1 :
        ...
        visual(fig_name_prefix=fig_name_prefix,
               ...
               tr_statistics=tr_statistics,
               err_dict=err_dict,
               feature_importance_inner=Fi_inner,
               feature_importance_outer=0
               )
...
def eval_epoch(model, validation_data, pred_loss_func, epoch, tr_statistics, config):
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2,
            ...
            for idx, _ in enumerate(covariates):
                covariates[idx] = [inst.to(config.device) for inst in covariates[idx]]
            """ forward """
            enc_out, prediction, _= model(event_type, event_time, covariates)
    ...
    for i, batch in enumerate(validation_data):
        ...
        temp_att = torch.zeros(size=(batch, seq_len, model.d_covar), device=covariates[0][0].device)
        for b_id, seq in enumerate(covariates):
            # print(len(seq), type(seq), 'insight seq!')
            len_seq = len(seq)
            temp_att[b_id, :, :] = torch.cat(seq).reshape(len_seq, model.d_covar)
        temp_att = temp_att[~torch.all(temp_att==0, axis=-1)]
        att = model.encoder.cov_intra_att.get_attention(temp_att)
        ATT.append(att)
...
def train(model, training_data, validation_data, optimizer, scheduler, pred_loss_func, tr_statistics, config):
    ...
    for epoch_i in range(config.epoch):
        ...
        # print(np.unique(pred_type), np.unique(next_type), 'hehe!!! type pred!')
        next_type = np.array(next_type) -1
        # F1_  = f1_score(next_type, pred_type, average=None)

        F1_  = f1_score(next_type, pred_type, average='weighted')
        F1_macro  = f1_score(next_type, pred_type, average='macro')
        print('(Training)-Weighted-F1_score: {} and macro: {}'.format(F1_, F1_macro))
...
def main():
    ...
    parser.add_argument('-delete_num', type=int, default=0)
